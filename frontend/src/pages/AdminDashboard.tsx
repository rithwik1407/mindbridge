  ...
  const handleExport = async (format: "csv" | "pdf") => {
    try {
      ...
      const ensurePageSpace = (lineCount = 1) => {
        if (y + lineCount * lineHeight > pageHeight - bottom) {
          doc.addPage();
          y = top;
        }
      };
      ...
      const writeText = (
        text: string,
        options: { bold?: boolean; size?: number; gapBefore?: number; gapAfter?: number } = {}
      ) => {
        const { bold = false, size = 11, gapBefore = 0, gapAfter = 0 } = options;
        if (gapBefore > 0) {
          y += gapBefore;
        }
      ...
      writeText("MindBridge Admin Report", { bold: true, size: 18, gapAfter: 4 });
      writeText(`Generated: ${generatedAt}`, { size: 11, gapAfter: 8 });

      writeText("Overview Dashboard", { bold: true, size: 13, gapAfter: 2 });
      writeText(`Total Youth: ${stats.totalYouth}`);
      writeText(`Active Chats: ${stats.activeChats}`);
      writeText(`Total Sessions: ${stats.totalSessions}`);
      writeText(`Active Crisis Flags: ${stats.activeCrisisFlags}`);

      writeText("Volunteer Management", { bold: true, size: 13, gapBefore: 8, gapAfter: 2 });
      if (volunteers.length === 0) {
        writeText("1. No volunteer requests found.");
      } else {
        volunteers.forEach((volunteer, index) => {
          const username = volunteer.user?.username || "Unknown";
          const email = volunteer.user?.email || "No email";
          const approval = volunteer.approvalStatus || "pending";
          const sessions = volunteer.performance?.totalSessions ?? 0;
          writeText(`${index + 1}. ${username} (${email}) - ${approval} - sessions ${sessions}`);
        });
      }

      writeText("Youth Case Status", { bold: true, size: 13, gapBefore: 8, gapAfter: 2 });
      if (cases.length === 0) {
        writeText("1. No youth cases found.");
      } else {
        cases.forEach((item, index) => {
          writeText(`${index + 1}. ${item.username} - ${item.caseStatus}`);
        });
      }
